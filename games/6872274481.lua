...
run(function()
	...
	local function isModuleEnabled(module)
		return type(module) == "table" and module.Enabled
	end

	Speed = vape.Categories.Blatant:CreateModule({
		Name = 'Speed',
		Function = function(enabled)
			frictionTable.Speed = enabled and true or nil
			updateVelocity()

			pcall(function()
				debug.setconstant(bedwars.WindWalkerController.updateSpeed, 7, enabled and 'constantSpeedMultiplier' or 'moveSpeedMultiplier')
			end)
			...
			if enabled then
				Speed:Clean(runService.PreSimulation:Connect(function(dt)
					...
					if entitylib.isAlive and isnetworkowner(entitylib.character.RootPart) then
						if isModuleEnabled(Fly) or isModuleEnabled(LongJump) then return end
						...
						local velo = getSpeed()
						local moveDir = AntiFallDirection or hum.MoveDirection
						local destination = moveDir * math.max(Value.Value - velo, 0) * dt
						...
						if AutoJump.Enabled and (state == Enum.HumanoidStateType.Running or state == Enum.HumanoidStateType.Landed) and moveDir ~= Vector3.zero and (Attacking or AlwaysJump.Enabled) then
